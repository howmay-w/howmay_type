...
- 根目錄 `fly.toml` 預設 **`SKIP_OPTIMIZE_IMAGES=true`**、VM **512MB**：不在 Fly 容器內跑圖片壓縮，改由 **GitHub Actions** 壓縮。若 **`git push` 出現 OOM**（malloc failed、tried to allocate 1048576000 bytes），常見原因是本機 `git config http.postBuffer` 設成 **1GB** 並被複製進 Docker 映像；`fly-entrypoint.sh` 會在啟動時改為 **50MB**。若仍失敗，再把 `memory` 改為 `1gb` 後 deploy。若需在本機 Bot 流程內壓圖，可設 `SKIP_OPTIMIZE_IMAGES=false` 並調高 VM 記憶體。
